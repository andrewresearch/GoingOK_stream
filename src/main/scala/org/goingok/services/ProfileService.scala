...
object ProfileService {
  ...
  def getResearchForGoingokId(goingok_id:UUID):Future[Research] = {
    DatabaseOps.selectUserForGoingokId(goingok_id).map{ user =>
      if(user.nonEmpty) buildResearchFromCode(user.get.research_code)
      else Research()
    }
  }

  private def buildResearchFromCode(code:String):Research = {
    val segs = code.split('-')
    Research(ResearchEntry("",segs.apply(0)),ResearchEntry("",segs.apply(1)),ResearchEntry("",segs.apply(2)))
  }
  ...
  def saveResearch(goingok_id:String,research:Research): Future[String] = {
    ...
    val code = research.project.code +"-"+ research.organisation.code +"-"+ research.cohort.code
      DatabaseOps.updateUser(UUID.fromString(goingok_id),code)
